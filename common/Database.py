...
def adapt_and_convert():
    ...
    def adapt_experiment(exp : Experiment):
        return f"{exp.version};{exp.model};{exp.fusion};{exp.dataset};{exp.batch_size};{exp.rounds};{exp.epochs};{exp.sample_fraction};{exp.proximal_mu}"
    
    sqlite3.register_adapter(Experiment, adapt_experiment)
    
    def convert_experiment(val : str):
        version, model, fusion, dataset, batch_size, rounds, epochs, sample_fraction, proximal_mu = val.split(';')
        return Experiment(model, fusion, dataset, int(batch_size), int(rounds), int(epochs), float(proximal_mu), float(sample_fraction), version)
